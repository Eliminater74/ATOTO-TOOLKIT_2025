...
object LauncherUtils {
    ...
    /** Best-effort current HOME app label (non-root). */
    suspend fun currentHomeLabel(ctx: Context): String? = withContext(Dispatchers.IO) {
        ...
        // If the resolver is shown, there is no fixed default yet.
        if (ri.activityInfo?.packageName == "android") return@withContext null

        runCatching { ri.loadLabel(pm).toString() }.getOrNull()
    }

    /** Non-root: show Android’s default-home picker (user can tap “Always”). */
    fun promptHomeChooser(ctx: Context) {
    ...
    /** Non-root: open Settings → Default apps → Home app. */
        ...
        withContext(Dispatchers.IO) {
            ...
            holders.filter { it != pkg }.forEach { other ->
                ...
                if (alsoDisableOthers) {
                    // Try uninstall for user 0; if fails, disable for user 0
            ...
            // Kick HOME so change applies immediately
